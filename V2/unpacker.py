...
class NestedUnpacker:
    ...
    def expand_archive(
        ...
        if fmt == "zip":
            ...
            self.log(f"[识别] {archive_name} -> {format_label(fmt)}")
            ...
            if password_pool_contains_keyword(self.passwords, MP4_EMBEDDED_ZIP_KEYWORD):
                ...
                self.log("[兜底] 密码池不包含“孙笑川”，但检测到 MP4 内嵌 ZIP，尝试 WinRAR")
        self.log(
    ...
    def expand_normal_archive_from_path(
        ...
        # MP4 内嵌 ZIP 必须保留为硬盘文件，才能通过“改名为 .zip -> WinRAR”处理。
    ...
    def extract_archive_to_dir(
        ...
        1. mp4_embedded_zip：
           ...
           只做一件事：原文件改名为 .zip -> WinRAR 解压。
           成功后删除 .zip；失败后恢复 .mp4 原名。

        2. 普通 zip：
           先 Python zipfile。
           Python 失败后用 7z 兜底。
           非 MP4 ZIP 不调用 WinRAR。
        ...
        if fmt == "mp4_embedded_zip":
            ...
            self.extract_mp4_embedded_zip_with_winrar(
                archive_name=archive_name,
                extract_dir=extract_dir,
                source_path=source_path,
        ...
        if fmt == "hidden_zip":
            raise RuntimeError(
                ...
                "WinRAR 只允许用于 archive_detector 识别出的 MP4 内嵌 ZIP。"
            )
    ...
    def extract_with_7z_engine(
        ...
        try:
            ...
            if result.success:
                self.log(f"[完成] 7z 解压成功：{archive_name}")
                return
                    ...
                    if (
                        ...
                        try:
                            ...
                            if child_fmt in {"hidden_zip", "mp4_embedded_zip"}:
                                # MP4 内嵌 ZIP 必须落成硬盘文件后改名 .zip，再交给 WinRAR。
                                dest = unique_path(out_path)
                                dest.write_bytes(data)
                                self.expand_archive(
    ...
    def extract_mp4_embedded_zip_with_winrar(
        self,
        archive_name: str,
        extract_dir: Path,
        source_path: Path,
        ...
        if not self.winrar_path:
            raise RuntimeError("找不到 WinRAR。MP4 内嵌 ZIP 特殊逻辑需要 WinRAR。")
        ...
        if alias_path.exists():
            raise RuntimeError(f"无法处理 MP4 内嵌 ZIP，目标别名已存在，避免覆盖：{alias_path}")

        if embedded_zip_offset is not None:
            mb = embedded_zip_offset / (1024 * 1024)
            self.detail(f"[MP4内嵌ZIP] offset={embedded_zip_offset} bytes ({mb:.2f} MB) file={archive_name}")

        renamed = False
        try:
            source_path.rename(alias_path)
            renamed = True
            self.log(f"[改名] {source_path.name} -> {alias_path.name}")
            self.log(f"[引擎] WinRAR：{alias_path.name}")

            try:
                self.run_winrar_extract(alias_path, extract_dir)
            except Exception as exc:
                self.log(f"[失败] WinRAR 解压失败：{alias_path.name}")
                try:
                    if alias_path.exists() and not source_path.exists():
                        alias_path.rename(source_path)
                        renamed = False
                        self.log(f"[恢复] 已恢复文件名：{source_path.name}")
                except Exception as restore_exc:
                    raise RuntimeError(
                        f"[严重] WinRAR 解压失败，且恢复原 MP4 文件名失败：{alias_path}\n"
                        f"WinRAR 错误：{exc}\n"
                        f"恢复错误：{restore_exc}"
                    ) from restore_exc

                raise RuntimeError(f"WinRAR 解压失败：{alias_path}\n{exc}") from exc

            self.log(f"[完成] WinRAR 解压成功：{alias_path.name}")
            try:
                if alias_path.exists():
                    alias_path.unlink()
                    renamed = False
                    self.log(f"[清理] 已删除：{alias_path.name}")
            except Exception as exc:
                raise RuntimeError(f"WinRAR 解压成功，但删除临时 .zip 失败：{alias_path}\n{exc}") from exc

        finally:
            if renamed and alias_path.exists() and not source_path.exists():
                try:
                    alias_path.rename(source_path)
                    self.log(f"[恢复] 已恢复文件名：{source_path.name}")
                except Exception as exc:
                    raise RuntimeError(f"[严重] 恢复原 MP4 文件名失败：{alias_path}\n{exc}") from exc
