...
WINDOW_TITLE = "多层嵌套压缩解压工具"
WINDOW_SIZE = "560x390"
DEFAULT_PASSWORD = "孙笑川258"
STARTUP_NOTES = (
    "【运行说明】本工具用于多层嵌套压缩文件自动解压，最多处理 11 层。",
    "【支持范围】ZIP、7Z、TAR、GZ、TGZ、BZ2、XZ、无扩展名、错误扩展名和常见分卷。",
    "【特殊规则】密码池包含“孙笑川”时，会优先检查 MP4 内嵌 ZIP；该场景使用 WinRAR，其余格式使用 Python / 7z。",
    "【注意】密码池支持多个密码，用中文或英文逗号分隔。",
)
...
def get_engine_status() -> tuple[str, list[str]]:
    ...
    winrar = unpacker.find_winrar()

    if seven_zip is not None:
        seven_status = "7z 已就绪"
        seven_log = f"[引擎状态] 7z：已就绪，路径：{seven_zip}"
    else:
        seven_status = "7z 未找到"
        seven_log = "[引擎状态] 7z：未找到，7Z/TAR/GZ/分卷功能不可用"

    if winrar:
        winrar_status = "WinRAR 已找到"
        winrar_log = f"[引擎状态] WinRAR：已找到，路径：{winrar}"
    else:
        winrar_status = "WinRAR 未找到，MP4 内嵌 ZIP 功能不可用"
        winrar_log = "[引擎状态] WinRAR：未找到，MP4 内嵌 ZIP 功能不可用"

    return f"引擎状态：{seven_status} | {winrar_status}", [seven_log, winrar_log]
...
class UnpackerGui:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(path_frame, text="目标", width=7).grid(row=0, column=0, sticky="w", padx=(0, 6))
        ...
        ttk.Label(output_frame, text="输出目录", width=7).grid(row=0, column=0, sticky="w", padx=(0, 6))
        ...
        ttk.Button(output_frame, text="默认原目录", width=10, command=self._clear_output_folder).grid(row=0, column=3, padx=(6, 0))

        control_frame = ttk.Frame(self.root, padding=(10, 0, 10, 8))
        control_frame.grid(row=2, column=0, sticky="ew")
        control_frame.columnconfigure(1, weight=1)

        ttk.Label(control_frame, text="密码池", width=7).grid(row=0, column=0, sticky="w", padx=(0, 6))
        ...
        engine_frame = ttk.Frame(self.root, padding=(10, 0, 10, 6))
