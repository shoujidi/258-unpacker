...
from archive_7z_engine import find_7z_executable
import unpacker


WINDOW_TITLE = "多层压缩解压工具"
WINDOW_SIZE = "430x300"
WINDOW_MIN_SIZE = (400, 280)
FORM_LABEL_WIDTH = 4
FORM_LABEL_PADX = (0, 4)
...
STARTUP_NOTES = (
    "【运行说明】用于多层压缩文件自动解压，最多处理6层。",
    "【支持范围】ZIP、7Z、错误扩展名和常见分卷。",
    "【特殊规则】支持孙笑川258资源",
    "【注意事项】支持多个密码，用逗号分隔。",
)
...
def get_engine_status() -> tuple[str, list[str]]:
    seven_zip = find_7z_executable()

    if seven_zip is not None:
        status = "7z 已就绪"
        log_line = f"【引擎状态】7z已就绪 {seven_zip}"
    else:
        status = "7z 未找到"
        log_line = "【引擎状态】 7z未找到，7Z/TAR/GZ/分卷功能不可用"

    return f"引擎状态：{status}", [log_line]
...
class UnpackerGui:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.engine_status = tk.StringVar()
    ...
    def _build_ui(self) -> None:
        ...
        path_frame = ttk.Frame(self.root, padding=(10, 6, 10, 2))
        path_frame.grid(row=0, column=0, sticky="ew")
        path_frame.columnconfigure(1, weight=1)

        ttk.Label(path_frame, text="目标", width=FORM_LABEL_WIDTH).grid(row=0, column=0, sticky="w", padx=FORM_LABEL_PADX)
        ttk.Entry(path_frame, textvariable=self.selected_path, width=30).grid(row=0, column=1, sticky="ew")
        ttk.Button(path_frame, text="文件", width=SMALL_BUTTON_WIDTH, command=self._choose_file).grid(row=0, column=2, padx=(6, 0))
        ttk.Button(path_frame, text="文件夹", width=WIDE_BUTTON_WIDTH, command=self._choose_folder).grid(row=0, column=3, padx=(6, 0))

        output_frame = ttk.Frame(self.root, padding=(10, 0, 10, 2))
        output_frame.grid(row=1, column=0, sticky="ew")
        output_frame.columnconfigure(1, weight=1)

        ttk.Label(output_frame, text="输出", width=FORM_LABEL_WIDTH).grid(row=0, column=0, sticky="w", padx=FORM_LABEL_PADX)
        ttk.Entry(output_frame, textvariable=self.output_path, width=30).grid(row=0, column=1, sticky="ew")
        ttk.Button(output_frame, text="选择", width=WIDE_BUTTON_WIDTH, command=self._choose_output_folder).grid(row=0, column=2, padx=(6, 0))

        control_frame = ttk.Frame(self.root, padding=(10, 0, 10, 4))
        control_frame.grid(row=2, column=0, sticky="ew")
        control_frame.columnconfigure(1, weight=1)

        ttk.Label(control_frame, text="密码", width=FORM_LABEL_WIDTH).grid(row=0, column=0, sticky="w", padx=FORM_LABEL_PADX)
        ...
        engine_frame = ttk.Frame(self.root, padding=(10, 0, 10, 2))
        engine_frame.grid(row=3, column=0, sticky="ew")
        engine_frame.columnconfigure(0, weight=1)
        ttk.Label(engine_frame, textvariable=self.engine_status).grid(row=0, column=0, sticky="w")
    ...
    def _append_startup_notes(self) -> None:
        engine_status, engine_logs = get_engine_status()
        self.engine_status.set(engine_status)
    ...
    def _prepare_ramdisk(self) -> bool:
        if not self.use_ramdisk.get():
            ...
            self._append_log("[RAM Disk] 已关闭，使用普通临时目录")
            return True
...
def main() -> None:
    unpacker.install_exit_cleanup_handlers()
    try:
        runtime_dir = Path(getattr(sys, "_MEIPASS")).resolve().parent
        if runtime_dir.name == ".runtime_cache":
            hide_folder_if_exists(runtime_dir)
    except Exception:
        pass
    set_windows_app_id()
