...
MAX_NESTED_DEPTH = 11
MAX_UNPACK_DEPTH = MAX_NESTED_DEPTH
...
GREEN_CLEANUP_DIR_NAMES = ("cache", ".runtime_cache", "._nested_unpacker_work", "__pycache__")
GREEN_RUNTIME_MARKER = ".archive258-runtime"

# MP4 内嵌 ZIP：保留 detector 判断，只替换处理方式为 offset 提取 + Python zipfile。
MP4_EMBEDDED_ZIP_USE_CARVE_PYZIP = True
MP4_EMBEDDED_ZIP_DELETE_TEMP_AFTER_SUCCESS = True
MP4_EMBEDDED_ZIP_KEEP_TEMP_ON_FAIL = False
MP4_CARVE_COPY_CHUNK_SIZE = 8 * 1024 * 1024
...
- 支持 MP4 内嵌 ZIP：从 detector 给出的 offset 提取 ZIP 有效段到临时目录，再用 Python zipfile 解压。
...
依赖：
- Python 3.10+
- ZIP 可直接处理，失败后可由 7z 兜底。
- 7Z / 分卷和若干 7z 可处理格式需要 7-Zip 命令行。
"""
...
import archive_7z_engine
from archive_7z_engine import extract_with_7z_password_pool
from archive_detector import (
    MP4_EMBEDDED_ZIP_KEYWORD,
    analyze_archive_candidate,
    password_pool_contains_keyword,
)
...
SEVEN_ZIP_ENGINE_FORMATS = SEVEN_ZIP_FORMATS | SPLIT_FORMATS
...
def cleanup_green_runtime_artifacts() -> None:
    if not PURE_GREEN_MODE:
        return
    ...
    try:
        ...
        direct_targets.add(meipass)
        if meipass.parent.name == ".runtime_cache":
            direct_targets.add(meipass.parent)
    except Exception:
        pass
    ...
    for root in roots:
        for name in GREEN_CLEANUP_DIR_NAMES:
            path = root / name
            try:
                if path.is_dir() and (path / GREEN_RUNTIME_MARKER).exists():
                    shutil.rmtree(path, ignore_errors=True)
            except Exception:
                pass


def mark_green_runtime_dir(path: Path) -> None:
    if not PURE_GREEN_MODE:
        return
    try:
        path.mkdir(parents=True, exist_ok=True)
        (path / GREEN_RUNTIME_MARKER).write_text("archive258 runtime\n", encoding="utf-8")
    except Exception:
        pass


def mark_pyinstaller_runtime_dir() -> None:
    if not PURE_GREEN_MODE:
        return
    try:
        meipass = Path(getattr(sys, "_MEIPASS")).resolve()
        ...
        return
    if meipass.parent.name == ".runtime_cache":
        mark_green_runtime_dir(meipass.parent)
...
def release_memory_now() -> None:
    ...
    try:
        ...
        cleanup_green_runtime_artifacts()
        trim_process_memory()
    finally:
        _CLEANUP_RUNNING = False
...
def collect_volume_set(first_path: Path) -> list[Path]:
    ...
    # xxx.z01 / xxx.z02 / xxx.zip；7z 应打开 xxx.zip
...
def is_mp4_extension(name: str) -> bool:
    return name.lower().endswith(".mp4")


def is_zip_candidate_by_detector(path: Path, passwords: list[str] | None = None) -> bool:
    try:
        info = analyze_archive_candidate(path, passwords=passwords or [])
        return info.is_archive and info.real_format == "zip" and info.engine_hint == "python_zip"
    except Exception:
        return False


def normalize_detector_format(real_format: str) -> str | None:
    if real_format == "zip":
        return "zip"
    if real_format == "mp4_embedded_zip":
        return "mp4_embedded_zip"
    if real_format in SEVEN_ZIP_FORMATS:
        return real_format
    if real_format in SPLIT_FORMATS:
        return real_format
    return None


def detect_format_by_detector(path: Path, passwords: list[str] | None = None) -> str | None:
    try:
        info = analyze_archive_candidate(path, passwords=passwords or [])
    except Exception:
        return None
    ...
    if not info.is_archive:
        return None
    return normalize_detector_format(info.real_format)


def get_archive_info(path: Path, passwords: list[str] | None = None):
    try:
        return analyze_archive_candidate(path, passwords=passwords or [])
    except Exception:
        return None


def is_split_format(fmt: str | None) -> bool:
    return fmt in SPLIT_FORMATS


def is_mp4_embedded_zip_format(fmt: str | None) -> bool:
    return fmt == "mp4_embedded_zip"


def split_skip_reason(path: Path) -> str:
    info = get_archive_info(path)
    if info is not None and info.skip_reason:
        if info.real_format == "split_zip_legacy":
            return "传统 ZIP 分卷数据卷，等待 .zip 入口"
        if info.real_format in {"split_rar_new", "split_rar_old"}:
            return "非入口分卷"
        return info.skip_reason
    if re.search(r"(?i)\.part\d+\.rar$", path.name):
        return "非入口分卷"
    if re.search(r"(?i)\.r\d{2,}$", path.name):
        return "非入口分卷"
    if re.search(r"(?i)\.z\d{2}$", path.name):
        return "传统 ZIP 分卷数据卷，等待 .zip 入口"
    return "非第一卷分卷"


def split_followup_volumes(path: Path) -> list[Path]:
    volumes = collect_volume_set(path)
    return [p for p in volumes if p != path]


def file_process_key(path: Path) -> tuple[str, int, int] | None:
    try:
        stat = path.stat()
        return (str(path.resolve()).lower(), int(stat.st_size), int(stat.st_mtime))
    except Exception:
        return None


def detect_format_by_name(name: str) -> str | None:
...
def detect_format_from_bytes(name: str, data: bytes) -> str | None:
    by_name = detect_format_by_name(name)
    if by_name is not None:
        return by_name

    if data.startswith(SEVEN_Z_MAGIC):
        return "7z"
    if data.startswith(GZ_MAGIC):
        return "gz"
    if data.startswith(BZ2_MAGIC):
        return "bz2"
    if data.startswith(XZ_MAGIC):
        return "xz"
    if data.startswith(RAR4_MAGIC) or data.startswith(RAR5_MAGIC):
        return "rar"
...
def format_label(fmt: str) -> str:
    labels = {
        ...
        "tar_xz": "TAR.XZ",
        "split_7z": "7Z 分卷入口",
        "split_zip": "ZIP 数字分卷入口",
        "split_numeric": "数字分卷入口",
        "split_zip_legacy": "传统 ZIP 分卷入口",
        "split_rar_new": "分卷入口",
        "split_rar_old": "分卷入口",
        "mp4_embedded_zip": "MP4 内嵌 ZIP",
        "zip": "ZIP",
        "hidden_zip": "HIDDEN_ZIP",
    }
    return labels.get(fmt, fmt.upper())


def display_archive_type(fmt: str) -> str:
    if fmt in {"split_rar_new", "split_rar_old"}:
        return "分卷入口"
    if fmt == "rar":
        return "压缩包"
...
def name_matches_format(name: str, fmt: str) -> bool:
    lower_name = name.lower()
    if fmt == "split_rar_new":
        return re.search(r"\.part0*1\.rar$", lower_name) is not None
    expected_suffixes = {
        "7z": (".7z",),
        "tar": (".tar",),
        "gz": (".gz",),
        ...
        "split_zip": (".zip.001",),
        "split_numeric": (".001",),
        "split_zip_legacy": (".zip",),
        "split_rar_old": (".rar",),
        "mp4_embedded_zip": (".mp4",),
    }
    return any(lower_name.endswith(suffix) for suffix in expected_suffixes.get(fmt, ()))
...
def name_suggests_archive(name: str) -> bool:
    ...
    if is_zip_like_extension(n):
        return True
    if detect_format_by_name(n) is not None:
        return True
    if n.endswith((".rar", ".7z", ".gz", ".bz2", ".xz", ".tgz")):
        return True
...
def detect_format(
    ...
    - mp4_embedded_zip
    - 7z
    - tar / gz / bz2 / xz / tgz / tar_gz / tar_bz2 / tar_xz
    - split
    - None
    """
    lower_name = name.lower()
    ...
    if path is not None:
        detector_fmt = detect_format_by_detector(path)
        if detector_fmt is not None:
            return detector_fmt

    if data is not None:
        data_name_fmt = detect_format_by_name(name)
        if data_name_fmt is not None:
            return data_name_fmt

    if path is not None and is_first_split_volume(path):
        if re.search(r"\.part0*1\.rar$", lower_name):
            return "split_rar_new"
        if lower_name.endswith(".rar") and path.with_suffix(".r00").exists():
            return "split_rar_old"
        return "split_numeric" if re.search(r"\.\d{3}$", lower_name) else "split_7z"

    if re.search(r"\.(7z|zip)\.001$", lower_name):
        return "split_7z" if ".7z." in lower_name else "split_zip"

    if re.search(r"\.001$", lower_name):
    ...
    try:
        if data is not None:
            if zipfile.is_zipfile(io.BytesIO(data)):
                return "mp4_embedded_zip" if is_mp4_extension(name) else "zip"
    ...
    if data is not None:
        data_fmt = detect_format_from_bytes(name, data)
        if data_fmt is not None:
            return data_fmt

    name_fmt = detect_format_by_name(name)
    if name_fmt is not None:
        return name_fmt

    if head.startswith(SEVEN_Z_MAGIC):
        return "7z"

    if head.startswith(GZ_MAGIC):
        return "gz"
...
class TempManager:
    ...
    def create_root_work(self, source_path: Path, output_dir: Path) -> Path:
        ...
        mark_green_runtime_dir(work_root)
...
class NestedUnpacker:
    def __init__(
        ...
        self.processed_keys: set[tuple[str, int, int]] = set()
        self.depth_limited_items: set[tuple[str, int]] = set()
        self.reported_depth_limited_items: set[tuple[str, int]] = set()
        ...
        self.log(f"[配置] 最大嵌套层数: {MAX_NESTED_DEPTH}")
    ...
    def depth_limited_key(self, path: Path) -> tuple[str, int] | None:
        try:
            return (path.name.lower(), int(path.stat().st_size))
        except Exception:
            return None

    def remember_depth_limited_path(self, path: Path) -> None:
        key = self.depth_limited_key(path)
        if key is not None:
            self.depth_limited_items.add(key)

    def is_depth_limited_path(self, path: Path) -> bool:
        key = self.depth_limited_key(path)
        return key is not None and key in self.depth_limited_items

    def report_depth_limited_once(self, path: Path) -> bool:
        key = self.depth_limited_key(path)
        if key is None:
            return True
        if key in self.reported_depth_limited_items:
            return False
        self.reported_depth_limited_items.add(key)
        return True
    ...
    def run(self, inputs: list[Path]) -> None:
        ...
        for index, p in enumerate(sorted(unique_roots, key=lambda x: str(x).lower()), start=1):
            ...
            if is_non_first_volume(p):
                self.total_skipped += 1
                self.log(f"[跳过] {p.name}：{split_skip_reason(p)}")
                continue
    ...
    def process_root_file(self, path: Path) -> None:
        ...
        try:
            ...
            if DELETE_ROOT_ARCHIVES_AFTER_SUCCESS and not is_split_format(fmt):
    ...
    def expand_archive(
        ...
        layer = depth + 1
        if layer > MAX_NESTED_DEPTH:
            self.total_skipped += 1
            self.log(f"[停止] 已达到最大嵌套层数 {MAX_NESTED_DEPTH}，后续压缩包不再继续解压：{archive_name}")
            return []
        ...
        if fmt in {"hidden_zip", "mp4_embedded_zip"} and not archive_name.lower().endswith(".zip"):
            display_name = archive_name + ".zip"

        size = self.source_size(fmt, source_path, source_data)
        self.log(f"[层级 {layer}/{MAX_NESTED_DEPTH}] 扫描：{archive_name}")
        if fmt == "zip":
            if is_zip_like_extension(archive_name):
                self.log(f"[识别] {archive_name} -> ZIP")
            else:
                self.log(f"[识别] {archive_name} -> ZIP，扩展名错误或无扩展名")
        elif fmt in SEVEN_ZIP_FORMATS:
            label = display_archive_type(fmt)
            if name_matches_format(archive_name, fmt):
                self.log(f"[识别] {archive_name} -> {label}")
            else:
                self.log(f"[识别] {archive_name} -> {label}，扩展名错误或无扩展名")
        elif fmt in SPLIT_FORMATS:
            self.log(f"[识别] {archive_name} -> {display_archive_type(fmt)}")
            if source_path is not None:
                followups = split_followup_volumes(source_path)
                if followups:
                    names = ", ".join(p.name for p in followups)
                    self.log(f"[分卷] 检测到后续分卷：{names}")
                else:
                    self.log(f"[分卷] {archive_name}：入口卷，未发现后续分卷，继续尝试")
        elif fmt == "mp4_embedded_zip":
            self.log(f"[识别] {archive_name} -> MP4 内嵌 ZIP 候选")
            info = get_archive_info(source_path, self.passwords) if source_path is not None else None
            if info is not None and info.embedded_zip_offset is not None:
                mb = info.embedded_zip_offset / (1024 * 1024)
                self.log(f"[扫描] 发现 PK\\x03\\x04，位置：{info.embedded_zip_offset} 字节，约 {mb:.2f} MB")
            if password_pool_contains_keyword(self.passwords, MP4_EMBEDDED_ZIP_KEYWORD):
                self.log("[规则] 密码池包含“孙笑川”，按特殊资源优先处理")
            else:
                self.log("[兜底] 密码池不包含“孙笑川”，但检测到 MP4 内嵌 ZIP，尝试提取 ZIP 有效段")
        self.log(
            f"[解压] depth={depth} type={display_archive_type(fmt)} "
        ...
        try:
            ...
            if delete_source_after and source_path is not None and not is_split_format(fmt):
    ...
    def source_size(
        ...
        try:
            if (fmt == "split" or is_split_format(fmt)) and is_first_split_volume(source_path):
        ...
        while True:
            ...
            for p in files:
                ...
                if depth + 1 > MAX_NESTED_DEPTH:
                    self.total_skipped += 1
                    self.remember_depth_limited_path(p)
                    self.log(f"[停止] 已达到最大嵌套层数 {MAX_NESTED_DEPTH}，后续压缩包不再继续解压：{p.name}")
                    continue

                if self.is_depth_limited_path(p):
                    if self.report_depth_limited_once(p):
                        self.total_skipped += 1
                        self.log(f"[跳过] 已达到最大嵌套层数 {MAX_NESTED_DEPTH}，保留不再解压：{p.name}")
                    continue

                if self.already_processed(p):
                    self.total_skipped += 1
                    self.log(f"[跳过] 已处理过：{p}")
                    continue

                self.log(f"[发现嵌套压缩包] depth={depth} type={display_archive_type(fmt)} file={p}")
                parent = p.parent
    ...
    def expand_normal_archive_from_path(
        ...
        # MP4 内嵌 ZIP 必须保留为硬盘文件，才能按 offset 提取 ZIP 有效段。
        # 不允许先读入内存，否则无法定位和流式提取原始文件区间。
        if (
            fmt != "hidden_zip"
            and not is_mp4_embedded_zip_format(fmt)
            and not is_split_format(fmt)
            and should_use_memory(size, "nested_archive", self.quiet)
        ):
    ...
    def extract_archive_to_dir(
        ...
        1. mp4_embedded_zip：
           例如 video.mp4 中间或尾部藏有 ZIP 数据。
           使用 detector 给出的 offset 提取 ZIP 有效段到本次临时目录，再用 Python zipfile 解压。
           成功或失败后都删除临时 ZIP，保持绿色运行。
        ...
        3. 其他格式：
           7Z / TAR / GZ / TGZ / BZ2 / XZ / 分卷使用 7z 引擎。
        """
        src_display = source_path if source_path else f"<memory:{archive_name}>"
        self.detail(
            f"[准备解压] type={display_archive_type(fmt)} "
            f"source={src_display} target={extract_dir}"
        )

        if fmt == "mp4_embedded_zip":
            if source_path is None:
                raise RuntimeError("MP4 内嵌 ZIP 必须来自硬盘文件，当前来源是内存数据，跳过特殊处理")

            info = get_archive_info(source_path, self.passwords)
            embedded_zip_offset = info.embedded_zip_offset if info is not None else None
            self.extract_mp4_embedded_zip_by_carve_pyzip(
                archive_name=archive_name,
                extract_dir=extract_dir,
                source_path=source_path,
        ...
        if fmt == "hidden_zip":
            raise RuntimeError(
                f"旧 hidden_zip 路线已禁用：{archive_name}。"
                "请使用 archive_detector 识别出的 MP4 内嵌 ZIP carve + pyzip 路径。"
            )

        if fmt == "zip":
            self.extract_zip_with_7z_fallback(
                archive_name=archive_name,
                extract_dir=extract_dir,
                source_path=source_path,
                source_data=source_data,
        ...
        if fmt in SEVEN_ZIP_ENGINE_FORMATS:
            self.extract_with_7z_engine(
                archive_name=archive_name,
                fmt=fmt,
                extract_dir=extract_dir,
                source_path=source_path,
        ...
        raise RuntimeError(
            f"当前专用工具只处理 ZIP / mp4_embedded_zip / 7z 支持格式，不处理格式：{fmt}，文件：{archive_name}"
        )

    def extract_with_7z_engine(
        self,
        archive_name: str,
        fmt: str,
        extract_dir: Path,
        ...
        archive_path: Path | None = source_path
        ...
        try:
            if archive_path is None:
                ...
                temp_name = safe_rel_path(archive_name).name or "archive"
                temp_source = self.temp_root / f"sevenzip_src_{abs(hash((archive_name, fmt, len(source_data)))) & 0xffffffff:x}_{temp_name}"
                ...
                self.log(f"[7z临时源] size={format_size(len(source_data))} path={temp_source}")
                archive_path = temp_source

            self.log(f"[引擎] 7z：{archive_name}")

            previous_7z_path = archive_7z_engine.SEVEN_ZIP_PATH
            try:
                if self.sevenzip_path:
                    archive_7z_engine.SEVEN_ZIP_PATH = self.sevenzip_path
                result = extract_with_7z_password_pool(archive_path, extract_dir, self.passwords)
            finally:
                archive_7z_engine.SEVEN_ZIP_PATH = previous_7z_path

            if result.success:
                self.log(f"[完成] 解压成功：{archive_name}")
                return

            error_text = result.error or result.stderr.strip() or result.stdout.strip()
            if is_split_format(fmt):
                self.log(f"[失败] 7z 分卷解压失败：{archive_name}")
            else:
                self.log(f"[失败] 7z 解压失败：{archive_name}")
            raise RuntimeError(
                f"7z 解压失败：{archive_path}\n"
                f"识别格式：{fmt}\n"
    ...
    def extract_zip_with_7z_fallback(
        self,
        archive_name: str,
        extract_dir: Path,
        source_path: Path | None,
        ...
        try:
            ...
            self.log(f"[完成] Python ZIP 解压成功：{archive_name}")
            return
        except Exception as python_error:
            self.log(f"[ZIP兜底] Python ZIP 解压失败：{python_error}")
            self.log(f"[ZIP兜底] 使用 7z 解压：{archive_name}")

            fallback_path: Path | None = source_path
            temp_source: Path | None = None
            ...
            try:
                if fallback_path is None:
                    ...
                    if not temp_name.lower().endswith(".zip"):
                        temp_name += ".zip"
                    temp_source = self.temp_root / f"zip_fallback_{abs(hash((archive_name, len(source_data)))) & 0xffffffff:x}_{temp_name}"
                    temp_source.parent.mkdir(parents=True, exist_ok=True)
                    temp_source.write_bytes(source_data)
                    self.estimated_temp_write_bytes += len(source_data)
                    self.log(f"[ZIP兜底] 临时落盘：{temp_source}")
                    fallback_path = temp_source

                previous_7z_path = archive_7z_engine.SEVEN_ZIP_PATH
                try:
                    if self.sevenzip_path:
                        archive_7z_engine.SEVEN_ZIP_PATH = self.sevenzip_path
                    result = extract_with_7z_password_pool(fallback_path, extract_dir, self.passwords)
                finally:
                    archive_7z_engine.SEVEN_ZIP_PATH = previous_7z_path

                if result.success:
                    self.log(f"[ZIP兜底] 7z 解压成功：{archive_name}")
                    return

                self.log(f"[失败] ZIP 解压失败：{archive_name}")
                raise RuntimeError(
                    f"[失败] ZIP 解压失败：{archive_name}\n"
                    f"Python ZIP 错误：{python_error}\n"
                    f"7z 错误：{result.error}"
                )
            finally:
                if temp_source is not None:
                    try:
                ...
                for index, info in enumerate(info_list, start=1):
                    ...
                    if (
                        ...
                        try:
                            ...
                            if child_fmt is not None and depth + 2 > MAX_NESTED_DEPTH:
                                self.total_skipped += 1
                                self.log(
                                    f"[停止] 已达到最大嵌套层数 {MAX_NESTED_DEPTH}，"
                                    f"后续压缩包不再继续解压：{rel.name}"
                                )
                                dest = unique_path(out_path)
                                dest.write_bytes(data)
                                self.remember_depth_limited_path(dest)
                                continue

                            if child_fmt in {"hidden_zip", "mp4_embedded_zip"}:
                                # MP4 内嵌 ZIP 必须落成硬盘文件后，再按 offset 提取 ZIP 有效段。
                                dest = unique_path(out_path)
                                dest.write_bytes(data)
                                self.expand_archive(
    ...
    def mp4_embedded_zip_cache_dir(self) -> Path:
        if self.temp_root is not None:
            return self.temp_root / "mp4_embedded_zip"
        work_root = Path.cwd() / "._nested_unpacker_work"
        mark_green_runtime_dir(work_root)
        return work_root / "mp4_embedded_zip"

    def carve_mp4_embedded_zip_to_cache(
        self,
        source_path: Path,
        embedded_zip_offset: int,
        archive_name: str,
    ) -> Path:
        if not source_path.exists():
            raise RuntimeError(f"MP4 内嵌 ZIP 来源不存在：{source_path}")
        if embedded_zip_offset < 0:
            raise RuntimeError(f"MP4 内嵌 ZIP offset 不合法：{embedded_zip_offset}")

        cache_dir = self.mp4_embedded_zip_cache_dir()
        cache_dir.mkdir(parents=True, exist_ok=True)

        base_name = safe_rel_path(archive_name).stem or "mp4_embedded_zip"
        temp_zip_path = unique_path(cache_dir / f"{base_name}_offset_{embedded_zip_offset}.zip")
        self._mp4_embedded_zip_last_temp_zip = temp_zip_path

        self.log(f"[提取] 从 offset {embedded_zip_offset} 开始复制到临时 ZIP")
        self.log(f"[临时ZIP] {temp_zip_path}")

        written = 0
        with source_path.open("rb") as src, temp_zip_path.open("wb") as out:
            src.seek(embedded_zip_offset)
            while True:
                self.checkpoint()
                chunk = src.read(MP4_CARVE_COPY_CHUNK_SIZE)
                if not chunk:
                    break
                out.write(chunk)
                written += len(chunk)

        self.estimated_temp_write_bytes += written
        if written <= 0:
            raise RuntimeError(f"MP4 内嵌 ZIP 提取结果为空：{source_path}")

        return temp_zip_path

    def extract_zip_member_with_password(
        self,
        zf: zipfile.ZipFile,
        info: zipfile.ZipInfo,
        extract_dir: Path,
        ...
        try:
            out_path.resolve().relative_to(extract_dir.resolve())
        except ValueError as exc:
            raise RuntimeError(f"ZIP 内存在不安全路径，已拒绝解压：{info.filename}") from exc
        ...
        try:
            with zf.open(info, "r", pwd=pwd_bytes) as src, out_path.open("wb") as out:
                shutil.copyfileobj(src, out, length=1024 * 1024)
        except Exception:
            try:
                if out_path.exists():
    ...
    def clear_extract_dir_contents(self, extract_dir: Path) -> None:
    ...
    def extract_temp_zip_with_pyzip(
        self,
        temp_zip_path: Path,
        extract_dir: Path,
        archive_name: str,
        ...
        last_error: Exception | None = None
        self.log("[Python ZIP] 尝试解压临时 ZIP")

        for password in password_candidates:
            self.checkpoint()
            try:
                self.clear_extract_dir_contents(extract_dir)
                with zipfile.ZipFile(temp_zip_path, "r") as zf:
                    for info in zf.infolist():
                        self.extract_zip_member_with_password(zf, info, extract_dir, password)
                return
            except Exception as exc:
                last_error = exc

        raise RuntimeError(f"Python zipfile 解压失败：{archive_name}，最后错误：{last_error}")

    def extract_mp4_embedded_zip_by_carve_pyzip(
        self,
        archive_name: str,
        extract_dir: Path,
        source_path: Path,
        embedded_zip_offset: int | None,
    ) -> None:
        if not MP4_EMBEDDED_ZIP_USE_CARVE_PYZIP:
            raise RuntimeError("MP4 内嵌 ZIP carve + pyzip 处理未启用")
        if not source_path.exists():
            raise RuntimeError(f"MP4 内嵌 ZIP 来源不存在：{source_path}")
        if embedded_zip_offset is None:
            raise RuntimeError(f"MP4 内嵌 ZIP 缺少 embedded_zip_offset：{archive_name}")

        mb = embedded_zip_offset / (1024 * 1024)
        temp_zip_path: Path | None = None
        self._mp4_embedded_zip_last_temp_zip = None

        self.log(f"[MP4内嵌ZIP] 使用提取有效 ZIP 段方式处理：{archive_name}")
        self.log(f"[扫描] PK\\x03\\x04 位置：{embedded_zip_offset} 字节，约 {mb:.2f} MB")

        try:
            temp_zip_path = self.carve_mp4_embedded_zip_to_cache(
                source_path=source_path,
                embedded_zip_offset=embedded_zip_offset,
                archive_name=archive_name,
            )
            self.extract_temp_zip_with_pyzip(
                temp_zip_path=temp_zip_path,
                extract_dir=extract_dir,
                archive_name=archive_name,
            )
            self.log(f"[完成] MP4 内嵌 ZIP 解压成功：{archive_name}")

            if MP4_EMBEDDED_ZIP_DELETE_TEMP_AFTER_SUCCESS and temp_zip_path.exists():
                temp_zip_path.unlink()
                self.log("[清理] 已删除临时 ZIP")
        except Exception as exc:
            if temp_zip_path is None:
                temp_zip_path = getattr(self, "_mp4_embedded_zip_last_temp_zip", None)
            self.log(f"[失败] MP4 内嵌 ZIP 提取或解压失败：{archive_name}")
            if temp_zip_path is not None and temp_zip_path.exists():
                try:
                    ...
                    pass
            raise RuntimeError(f"MP4 内嵌 ZIP 提取或解压失败：{archive_name}\n{exc}") from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="递归解压常见压缩包，支持嵌套、分卷、加密、隐藏 ZIP。"
    )
