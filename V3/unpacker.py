...
MP4_EMBEDDED_ZIP_KEEP_TEMP_ON_FAIL = True
...
- 支持 MP4 内嵌 ZIP：从 detector 给出的 offset 提取 ZIP 有效段到 cache，再用 Python zipfile 解压。
...
class NestedUnpacker:
    ...
    def expand_normal_archive_from_path(
        ...
        if (
            ...
            self.expand_archive(
                ...
                source_path=p,
                source_data=None,
                delete_source_after=not is_split_format(fmt),
                root_output_dir=root_output_dir,
            )
    ...
    def extract_archive_to_dir(
        ...
        1. mp4_embedded_zip：
           ...
           使用 detector 给出的 offset 提取 ZIP 有效段到 cache，再用 Python zipfile 解压。
           成功后删除临时 ZIP；失败后保留临时 ZIP。
    ...
    def extract_mp4_embedded_zip_by_carve_pyzip(
        ...
        try:
            ...
            if (
                MP4_EMBEDDED_ZIP_KEEP_TEMP_ON_FAIL
                and temp_zip_path is not None
                and temp_zip_path.exists()
            ):
                self.log(f"[保留] 临时 ZIP 已保留，便于人工检查：{temp_zip_path}")
            elif temp_zip_path is not None and temp_zip_path.exists():
                try:
                    temp_zip_path.unlink()
                except Exception:
                    pass
